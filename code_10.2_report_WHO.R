...
make_flextable_style <- function(
  ...
  # EXPLICITLY USE flextable:: HERE TO FIX THE ERROR
  ft <- flextable::font(ft, fontname = font_family, part = "all")
  ft <- flextable::fontsize(ft, size = font_size, part = "all")
  
  # align (scoped just to be safe from stats::signif or others)
  ...
  # EXPLICITLY USE flextable:: HERE TO PREVENT BASE R/BIOC GENERICS CONFLICTS
  ft <- flextable::width(ft, width = new_widths)
...
N_map <- c(
  inc = "RSV-associated ALRI episodes",
...
tiff_NR<-data.frame(
  ...
  mutate(caption=sprintf(if_else(type=='N','Number of %s in %s aged %s in LMICs','%s in %s aged %s in LMICs'),
                         if_else(type=='N',N_map[metric],R_map[metric]),
                         if_else(age %in% age_map[1:3],'infants','children'),
                         age),
         note=paste0("Color scale is based on log10-transformed values. ",
...
export_images_word <- function(
  ...
  head2<-recode(metric,
                'inc'='RSV-associated ALRI incidence by country',
                'hos'='RSV-associated ALRI hospital admission by country',
                'mort.ass'='RSV-associated all-cause deaths by country',
                'mort.att'='RSV-attributable all-cause deaths by country')
  
  doc<-doc %>%
    body_add_par(head2, style = "heading 2")
  
  start_index<-recode(metric,"inc" = 1, "hos" = 5, "mort.ass" = 13, "mort.att" = 9) 
...
tiff_NR %>%
  filter(age %in% c('0–<12 months','0–<60 months')) %>% 
  split(.,.$metric) %>%
  iwalk(~{
    df<-.x
    export_images_word(metric=.y,df$path,df$caption,df$note,
                       output_file = sprintf("docs/Figures_NR_map_%s.docx",.y))
  })
  

export_images_word_appendix <- function(
    metric,
    image_files,
    image_captions,
    image_notes,
  ...
  head1<-recode(metric,
                'inc'='Supplementary results of RSV-associated ALRI incidence',
                'hos'='Supplementary results of RSV-associated ALRI hospital admission',
                'mort.ass'='Supplementary results of RSV-associated all-cause deaths',
                'mort.att'='Supplementary results of RSV-attributable all-cause deaths')
  
  doc<-doc %>%
    body_add_par(head1, style = "heading 1")
  
  start_index<-recode(metric,"inc" = 8, "hos" = 14, "mort.att" =20, "mort.ass" = 26) 
  
  for (i in seq_along(image_files)) {
    
    fig_id <- start_index + i - 1
    
    caption <- paste0('Figure S',fig_id,'. ',image_captions[i])
    note <- image_notes[i]
    ...
    doc <- doc %>%
      body_add_par(caption, style = "heading 2") %>%
      body_add_img(src = image_files[i], width = width, height = height) %>%
      body_add_fpar(fpar(ftext(note, prop = fp_text(font.family = "Calibri", font.size = 10.5)))) %>%
      body_end_block_section(block_section(property = ps))
  }
...
tiff_NR %>%
  ...
  iwalk(~{
    df<-.x
    export_images_word_appendix(metric=.y,df$path,df$caption,df$note,
                       output_file = sprintf("docs/Appendix_Figures_NR_map_%s.docx",.y))
  })
...
export_images_word_ratio <- function(
    image_files,
    image_captions,
    image_notes,
    start_index = 1,
...
export_images_word_ratio(tiff_ratio$path,tiff_ratio$caption,tiff_ratio$note,start_index = 32,output_file = "docs/Figures_ratio_map.docx")
