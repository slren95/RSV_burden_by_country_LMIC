...
c('df_sum_all_DeCoDe','df_sum_all_NP') %>%
  walk(~{
    ...
    df_sum_all_DeCoDe %>%
      ...
      saveRDS(str_replace(filename,'\\.docx','\\.rds')) %>%
...
#                       )
#                     }),
#              across(contains('_R_'),~round(.x,2))) %>%
#       transmute(`Income Level`=paste0(Income2019,if_else(Income2019=='Global','','IC')),
#                 N_0006 = sprintf('%s (%s–%s)', m0006_N_500, m0006_N_025, m0006_N_975),
#                 N_0612 = sprintf('%s (%s–%s)', m0612_N_500, m0612_N_025, m0612_N_975),
#                 N_1260 = sprintf('%s (%s–%s)', m1260_N_500, m1260_N_025, m1260_N_975),
#                 N_0060 = sprintf('%s (%s–%s)', m0060_N_500, m0060_N_025, m0060_N_975),
#                 R_0006 = sprintf('%.2f (%.2f–%.2f)', m0006_R_500, m0006_R_025, m0006_R_975),
#                 R_0612 = sprintf('%.2f (%.2f–%.2f)', m0612_R_500, m0612_R_025, m0612_R_975),
#                 R_1260 = sprintf('%.2f (%.2f–%.2f)', m1260_R_500, m1260_R_025, m1260_R_975),
#                 R_0060 = sprintf('%.2f (%.2f–%.2f)', m0060_R_500, m0060_R_025, m0060_R_975)) %>%
#       mutate(
#         `0-<6m`  = paste(N_0006, R_0006, sep = "\n"),
#         `6-<12m`  = paste(N_0612, R_0612, sep = "\n"),
#         `12-<60m` = paste(N_1260, R_1260, sep = "\n"),
#         `Total`  = paste(N_0060, R_0060, sep = "\n")
#       ) %>%
#       select(`Income Level`, `0-<6m`, `6-<12m`, `12-<60m`, `Total`) %>%
#       export_flextable_word(filename,orientation='land')
#   })
...
RF.res.impute2 %>%
  ...
  select(Country,name,contains('-')) %>% 
  rename_with(~" ",.cols='name') %T>%  
  saveRDS('docs/Table_incidence.rds') %>%
...
# bind_rows(
#   RF.res.impute %>%
#     select(ISOCountry,Income2019,AGEGR,pop,starts_with('N.')) %>%
#     summarise(across(where(is.numeric),sum),.by=c(Income2019,AGEGR)),
#   RF.res.impute %>%
#     select(ISOCountry,Income2019,AGEGR,pop,starts_with('N.')) %>%
#     summarise(across(where(is.numeric),sum),.by=c(AGEGR)) %>% mutate(Income2019='LMIC')
# ) %>%
#   mutate(IR.est=N.est/pop,IR.lci=N.lci/pop,IR.uci=N.uci/pop) %>%
...
#   transmute(`Income Level`=paste0(Income2019,'IC'),`0-<6m`, `6-<12m`, `12-<60m`, `Total`=`0-<60m`) %>%
